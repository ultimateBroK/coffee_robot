...
void startCoffeeSequence() {
    ...
    robot.progress = 0.0f;
    printf("🤖 Bắt đầu quy trình pha cà phê...\n");
}

void resetRobot() {
    initRobot();
    printf("🔄 Robot đã được reset về vị trí ban đầu\n");
}
    ...
    switch (robot.phase) {
        ...
        case GRAB_CUP:
            ...
            if (robot.progress >= 1.0f) {
                ...
                robot.progress = 0.0f;
                printf("🔄 Đang xoay về phía máy pha cà phê...\n");
            }
            break;
        ...
        case GO_HOME: {
            ...
            if (robot.progress >= 1.0f) {
                ...
                robot.moving = false;
                printf("✅ Cà phê đã sẵn sàng! Nhấn R để reset, SPACE để pha thêm.\n");
            }
            break;
        }
...
void drawTable() {
    // Vẽ mặt bàn với màu nâu gỗ nhẹ nhàng, không chói mắt
    glPushMatrix();
    drawSimpleBox(10.0f, 0.2f, 8.0f, 0.55f, 0.35f, 0.25f);  // Màu nâu gỗ đậm hơn, đỡ chói
    glPopMatrix();
}
